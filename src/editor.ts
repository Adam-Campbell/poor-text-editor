 ...
 * 
 * Mouse logic:
 * - When the user clicks on the text, we need to determine the position of the cursor.
 * - If the user clicks and drags, we need to determine the selection range.
 * 
 * Selection logic:
 * - We need to add state and logic for handling selection ranges.
 * - Selections can be made by clicking and dragging, or by using the shift key in combination with the arrow keys (possibly
 *  also if you hold down the ctrl key, then it expands the selection by a word each time).
 * 
 * 
 * 
 * 
